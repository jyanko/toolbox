...
function ghwip() {
  ...
  gh issue list --json number,title,labels,updatedAt,projectItems | \
    jq -r '["ID", "TITLE", "STATUS", "LABELS", "UPDATED"], (.[] | . as $issue | (.projectItems[] | select(.status.name == "In progress" or .status.name == "In review")) as $project | [("#" + ($issue.number | tostring)), $issue.title, $project.status.name, (if ($issue.labels | length) > 0 then ($issue.labels | map(.name) | join(", ")) else "---" end), ($issue.updatedAt | fromdateiso8601 | now - . | if . < 86400 then (. / 3600 | floor | tostring) + " hours ago" else (. / 86400 | floor | tostring) + " days ago" end)]) | @tsv' | \
    column -t -s $'\t'
}
...
function ghtodo() {
  ...
  #
  gh issue list --json number,title,labels,updatedAt,projectItems | \
    jq -r '["ID", "TITLE", "STATUS", "LABELS", "UPDATED"], (.[] | . as $issue | (.projectItems[] | select(.status.name == "To-Do")) as $project | [("#" + ($issue.number | tostring)), $issue.title, $project.status.name, (if ($issue.labels | length) > 0 then ($issue.labels | map(.name) | join(", ")) else "---" end), ($issue.updatedAt | fromdateiso8601 | now - . | if . < 86400 then (. / 3600 | floor | tostring) + " hours ago" else (. / 86400 | floor | tostring) + " days ago" end)]) | @tsv' | \
    column -t -s $'\t'
}
...
function ghback() {
  ...
  #
  gh issue list --json number,title,labels,updatedAt,projectItems | \
    jq -r '["ID", "TITLE", "STATUS", "LABELS", "UPDATED"], (.[] | . as $issue | (.projectItems[] | select(.status.name == "Backlog")) as $project | [("#" + ($issue.number | tostring)), $issue.title, $project.status.name, (if ($issue.labels | length) > 0 then ($issue.labels | map(.name) | join(", ")) else "---" end), ($issue.updatedAt | fromdateiso8601 | now - . | if . < 86400 then (. / 3600 | floor | tostring) + " hours ago" else (. / 86400 | floor | tostring) + " days ago" end)]) | @tsv' | \
    column -t -s $'\t'
}
